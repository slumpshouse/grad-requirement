...
function ProgressBar({ value }) {
  ...
  return (
    <div className="lp-progress" role="progressbar" aria-valuenow={clamped} aria-valuemin={0} aria-valuemax={100}>
      <div className="lp-progress__fill" style={{ width: `${clamped}%` }} />
    </div>
  );
}
...
export default function LessonPage() {
  ...
  const fetchLesson = useCallback(async (selectedLevel) => {
    setLessonLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/lesson?level=${selectedLevel}`);
      if (!res.ok) throw new Error('API error');
      const data = await res.json();
      setLesson(data.lesson || null);
    } catch {
      // Fall back to mock data when API is unavailable
      setLesson(MOCK_LESSONS[selectedLevel] || MOCK_LESSONS.beginner);
    } finally {
  ...
  }, [user, level, fetchLesson, fetchWeakness]);
  ...
  const handleCompleteLesson = async () => {
    setCompleting(true);
    try {
      const body = lesson?.id ? { lessonId: lesson.id } : { lessonId: `level-${level}` };
      ...
      {lessonLoading ? (
        ...
        <div className="lp-content" data-testid="lesson-content">
          ...
          <header className="lp-header lp-card">
            ...
            <p className="lp-header__progress-label">{lesson.progress}% through this level</p>
          </header>

          {/* ── Grammar ────────────────────────────────────────────────────── */}
          <GrammarSection grammar={lesson.grammar} level={level} />

          {/* ── Vocabulary ─────────────────────────────────────────────────── */}
          <VocabularySection vocabulary={lesson.vocabulary} />

          {/* ── Sentence Breakdown ─────────────────────────────────────────── */}
          <SentenceBreakdown example={lesson.example} />
